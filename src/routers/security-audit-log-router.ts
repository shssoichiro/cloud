...
const ListSecurityAuditLogsInputSchema = z.object({
...
export const securityAuditLogRouter = createTRPCRouter({
  list: baseProcedure.input(ListSecurityAuditLogsInputSchema).query(async ({ input, ctx }) => {
    const {
      before,
      after,
      action,
    ...
    const whereConditions = [eq(security_audit_log.owned_by_user_id, ctx.user.id)];
  ...
  getActionTypes: baseProcedure.query(async () => {
  ...
  export: baseProcedure.input(ExportInputSchema).mutation(async ({ input, ctx }) => {
    const { format, startTime, endTime, action } = input;
