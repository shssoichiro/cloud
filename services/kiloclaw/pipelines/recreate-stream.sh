...
#   5. Update the pipeline ID in wrangler.jsonc
#   6. Deploy the worker
#
# Usage:
...
#     kiloclaw_events_pipeline_stream \
#     pipelines/events-schema.json \
#     kiloclaw_events_pipeline \
#     kiloclaw_events_pipeline_sink
#
#   ./pipelines/recreate-stream.sh \
#     kiloclaw_controller_telemetry_pipeline_stream \
#     pipelines/controller-telemetry-schema.json \
#     kiloclaw_controller_telemetry_pipeline \
#     kiloclaw_controller_telemetry_pipeline_sink
#
# Prerequisites:
...
NEW_PIPELINE_SQL="INSERT INTO ${SINK_NAME} SELECT * FROM ${STREAM_NAME}"
echo "==> Creating new pipeline '${PIPELINE_NAME}'..."
echo "    SQL: ${NEW_PIPELINE_SQL}"
npx wrangler pipelines create "$PIPELINE_NAME" --sql "$NEW_PIPELINE_SQL"

NEW_PIPELINE_ID=$(npx wrangler pipelines list --json 2>/dev/null \
	| jq -r --arg name "$PIPELINE_NAME" '.[] | select(.name == $name) | .id')

if [[ -z "$NEW_PIPELINE_ID" ]]; then
	echo "Error: failed to find newly created pipeline '${PIPELINE_NAME}'" >&2
	exit 1
fi
echo "    New pipeline ID: ${NEW_PIPELINE_ID}"

echo "==> Updating wrangler.jsonc (${PIPELINE_ID} -> ${NEW_PIPELINE_ID})..."
if ! grep -q "$PIPELINE_ID" wrangler.jsonc; then
	echo "Error: old pipeline ID ${PIPELINE_ID} not found in wrangler.jsonc" >&2
	exit 1
fi
sed -i.bak "s/${PIPELINE_ID}/${NEW_PIPELINE_ID}/g" wrangler.jsonc && rm -f wrangler.jsonc.bak
echo "    Updated."
...
echo ""
echo "Done. Pipeline '${PIPELINE_NAME}' recreated with new ID ${NEW_PIPELINE_ID}."
